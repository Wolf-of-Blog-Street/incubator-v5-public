...
export async function handleLocalCommand(command, parsed, roster, loadInstaller, rosterPath) {
  switch (command) {
    ...
    case 'project-catalog': {
      ...
      const boardsDir = roster.boardsDir || path.join(path.dirname(rosterPath), '../boards');
    ...
    case 'project': {
      ...
      if (sub === 'rm') {
        ...
        const r = roster.removeProject(agentId, projId);
        console.log(`\n🗑  Removed "${projId}" from ${agentId}${r.board ? ` (board file ${r.board} left on disk)` : ''}\n`);
        break;
      }
      if (sub !== 'add') {
    ...
    case 'notes': {
      ...
      if (parsed._.length > 2) {
        const updated = roster.updateAgent(agentId, { notes: parsed._.slice(2).join(' ') });
        console.log(`\n✅ Notes for "${agentId}": ${updated.notes || '(cleared)'}\n`);
      } else {
        const ag = roster.getAgent(agentId);
