...
export async function handleSyncDoc(board, parsed, projectName, extractMilestonesFromDoc) {
  ...
  // Auto-scaffold tasks from Implementation Milestones. Docs are evergreen: a new "### Round N"
  // block appended later scaffolds only its milestones that have no task on the board yet.
...
export async function handleFinishDoc(board, parsed, projectName) {
  ...
  if (!res.closed) {
    ...
    const updatedFile = updateDocFrontmatterStatus(slug, { status: 'Finished', projectName });
    if (updatedFile) {
      console.log(`Updated status to Finished in: ${updatedFile}`);
    }
...
export async function handleReopenDoc(board, parsed, projectName) {
  ...
  if (!res.reopened) {
    ...
    const updatedFile = updateDocFrontmatterStatus(slug, { status: 'Active', projectName });
    if (updatedFile) {
      console.log(`Updated status to Active in: ${updatedFile}`);
    }
