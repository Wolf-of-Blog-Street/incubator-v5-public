  ...
  return `
    <div class="swimlane-header">
      ...
      <div class="swimlane-header-right">
        ...
        ${!isClosed ? `<button class="btn btn-secondary btn-sm" data-action="cut-task-for-doc" data-slug="${escapeHtml(slug)}">+ Add task</button>` : ''}
 ...
 */
export function createSwimlaneColumnsHtml({ slug, todo, inProgress, review, done, isDoneColCollapsed }) {
  ...
  const ordered = latestFirst(done);
  const shown = isDoneColCollapsed ? ordered.slice(0, DONE_PREVIEW) : ordered;
  const moreBtn = done.length > DONE_PREVIEW
    ? `<button type="button" class="btn-show-more" data-action="toggle-done-col" data-slug="${escapeHtml(slug)}">${isDoneColCollapsed ? `Show all ${done.length} done` : 'Show fewer'}</button>`
    : '';
  return `
    <div class="swimlane-columns">
      ...
      <div class="swimlane-col ${isDoneColCollapsed ? 'is-collapsed' : ''}" data-col-status="done">
        <div class="swimlane-col-header"><span class="swimlane-col-title">Done</span><span class="col-count">${done.length}</span></div>
        <div class="swimlane-card-list">${shown.length ? shown.map(createCardHtml).join('') : '<div class="col-empty">Nothing done yet</div>'}${moreBtn}</div>
      </div>
    </div>
  `;
}
...
export function createStandaloneSwimlaneHtml(standaloneTasks = []) {
  ...
  const isStandaloneDoneCollapsed = isDoneCollapsed('standalone');
  ...
  const columnsHtml = createSwimlaneColumnsHtml({
    ...
    isDoneColCollapsed: isStandaloneDoneCollapsed
  });
  ...
  return `
    <div class="doc-swimlane standalone-swimlane" data-slug="standalone">
      <div class="swimlane-header">
        <div class="swimlane-header-left">
          <button type="button" class="swimlane-toggle-btn" data-action="toggle-swimlane-collapse" title="Collapse or expand" aria-label="Collapse or expand lane">▾</button>
          <h3 class="swimlane-title">Standalone tasks</h3>
        ...
        <div class="swimlane-header-right">
          ...
          <button class="btn btn-secondary btn-sm" data-action="cut-standalone-task">+ Add task</button>
        </div>
      </div>
      ${columnsHtml}
    </div>
...
export function renderSwimlanes(tasks) {
  ...
  // 3. Render Standalone Tasks section if present
  ...
  dom.swimlanesList.innerHTML = swimlanesHtml.length
    ? swimlanesHtml.join('')
    : '<div class="col-empty board-empty">No tasks on this board yet. Press N to add one.</div>';
