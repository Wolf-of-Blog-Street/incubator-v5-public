...
test('installHarness installs harness, brain, board, work zones, and preserves HARNESS-custom.md', () => {
  ...
  try {
    ...
    assert.match(cardText, /Mandatory Bootstrap Protocol/i);
    assert.match(cardText, /Check the Board & Design Specs First/i);
    ...
    installHarness({ agentHome: tmpAgentHome, upgradeCards: true });
    const upgradedCardText = fs.readFileSync(agentCardPath, 'utf8');
    assert.match(upgradedCardText, /Mandatory Bootstrap Protocol/i);
