...
# with your project's coder-prompt.md and reviewer-prompt.md), LOGS. Models: CODER_MODEL, REVIEWER_MODEL. Efforts: CODER_EFFORT, REVIEWER_EFFORT.
set -e
...
run() { # run <role> <model> <effort> <prompt-file>
  $FRIEND run claude --model "$2" --effort "$3" --no-jj --prompt-file "$4" --cwd "$WS_DIR/$JOB" --timeout 0 > "$LOGS/$JOB-$1.log" 2>&1
}
{ cat "$PROMPTS/coder-prompt.md"; echo; echo ---; echo; cat "$CARDS/$JOB.md"; } > "$LOGS/$JOB-coder-prompt.md"
run coder "${CODER_MODEL:-claude-opus-5-5}" "${CODER_EFFORT:-medium}" "$LOGS/$JOB-coder-prompt.md" || echo "coder exited non-zero; the reviewer judges the code alone"
...
run reviewer "${REVIEWER_MODEL:-claude-opus-5-5}" "${REVIEWER_EFFORT:-high}" "$LOGS/$JOB-review-prompt.md"
