...
1. **Read Master Harness**: Read [harness/HARNESS.md](HARNESS.md) to understand installed tools, engineering standards, work zones, and engine capabilities.
2. **Read Custom Preferences**: Read [harness/HARNESS-custom.md](HARNESS-custom.md) for custom operator preferences.
3. **Check Active Memory**: Check your brain (`node harness/engine/brain.mjs list`) to recall active state.
4. **Check the Board**: Inspect the project board (`node harness/components/board/tools/board.mjs list`) to see what tasks are in-progress.
...
- **GitHub is NOT Required**: Projects in `workspaces/<project>/` do NOT need a GitHub remote — some will have remotes, some are purely local JJ/Git repositories.
...
## 6. Work Zones: `projects/` vs. `workspaces/`

| Zone | Purpose | Board Tracking | Version Control | Structure |
| :--- | :--- | :--- | :--- | :--- |
| **`projects/`** | **Local-Only Work** | **No Board** (Small, fast, lightweight) | Local Git/JJ (No remote) | Scratch scripts, one-off tools, rapid prototypes. Has `projects/<name>/src/` and `projects/<name>/docs/`. |
| **`workspaces/`** | **Real Managed Projects** | **Dedicated Board** (`boards/<project>.sqlite`) | Local JJ/Git (GitHub optional) | Real production codebases. **1 unified repository containing source code and in-repo `docs/`. GitHub remote is NOT required.** |
...
## 8. Workflow: Evergreen Design Docs & Task Cards

### The 2-Layer Hierarchy:
1. **Evergreen Design Doc (`docs/design/<slug>.md`)**:
   - The living architectural specification of a component or subsystem.
   - **Design Doc States**:
     - **`Open & Active`**: Currently has tasks planned, in-progress, or in-review. Work is actively being designed, implemented, or tested.
     - **`Open & Inactive`**: All current tasks are `Done` (or no tasks are currently queued). The subsystem is stable, but the doc **remains Open and evergreen**. New tasks can be attached at any time to reactivate it.
     - **`Closed & Inactive`**: Subsystem has been permanently decommissioned or superseded. **Only the human operator can mark a doc `Closed`**. Only closed docs can be archived.
   - **The Non-Closing Invariant**: Having 0 tasks or finishing all tasks NEVER closes a doc—it simply shifts it from `Open & Active` to `Open & Inactive`. Agents are **STRICTLY FORBIDDEN** from marking a design doc `Closed` or archiving it.
2. **Task Cards on the Project Board (`boards/<project>.sqlite`)**:
   - Discrete, actionable units of work added under the component's design doc (`--doc <slug>`).
   - Tasks transition cleanly through: `Planned` -> `In Progress` -> `In Review` -> `Done`.
   - **Tasks are NEVER marked "Closed"** (`Closed` is a design doc lifecycle state, not a task state).

### Pair Programming Protocol:
1. **Author the Evergreen Design Doc**: Reference or create `docs/design/<slug>.md` following `templates/epic.template.md`, structure phases into `## Implementation Milestones`, and register in `docs/design/INDEX.md`.
2. **Atomic Board Scaffolding (`sync-doc`)**: In the exact same turn, run `node harness/components/board/tools/board.mjs sync-doc <slug> --project <project>`. This parses the milestones checklist, registers the doc on the board, and auto-scaffolds milestone tasks. Never leave a design doc on disk without syncing its task cards to the board.
3. **Milestone 1 Protocol (Setup & Yield-Turn)**:
   - Milestone 1 is ALWAYS: `Milestone 1: Setup the design doc and tasks`.
   - When scaffolded, Milestone 1 initializes in `🟢 GREEN` (`in-progress`), authorizing the initial doc setup and task creation, while Tasks 2+ initialize in `🔴 RED` (`planned`).
   - Attach the detailed plan/checklist to Milestone 1: `node harness/components/board/tools/board.mjs set <id> --details "<plan>"`.
   - Complete Milestone 1 setup and transition it to `🔍 In-Review`: `node harness/components/board/tools/board.mjs set <id> --status review`. This **automatically flips its gate to `🔴 RED`**.
   - Commit the design revision cleanly in Jujutsu (`jj describe -m "..." && jj bookmark set main -r @ && jj new`).
   - Yield the turn to the operator for architectural review and gate clearance (`🟢 GREEN`) of Milestone 2.
4. **Build, Test, and Verify**: Once un-gated, advance each milestone through `in-progress` -> `review` -> `done`, verifying with `npm test` and Chrome DevTools MCP.
5. **Review & Land**: Operator reviews each milestone in `review` and approves merge.
6. **Fold-Back on Landing**: Update `docs/system/ARCHITECTURE.md` if schemas changed, and update the evergreen design doc (`docs/design/<slug>.md`). The doc shifts from `Open & Active` to `Open & Inactive`.

### UX Execution Standard: Never Build Blind
All UX, frontend, web viewer, and UI tasks must be done with Chrome to check as you create. UXs should never be built blind:
...
```bash
# 1. Start a focused change off main with a descriptive commit message
jj new main -m "feat(area): short description"

# 2. Develop and test in the working copy
npm test

# 3. Advance / fast-forward main bookmark to the verified commit
...
**Why this standard**:
- Keeps a clean, bisectable, linear Git history without redundant merge commit bubbles.
