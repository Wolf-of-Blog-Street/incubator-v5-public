...
export async function handleAdminRoutes(req, res, pathname, ctxOrAppContext, ...legacyArgs) {
  ...
  const adminAgentPatchMatch = pathname.match(/^\/api\/v1\/admin\/agents\/([a-zA-Z0-9_\-]+)$/);
  if (method === 'PATCH' && adminAgentPatchMatch) {
    const agentId = adminAgentPatchMatch[1];
    if (!ctx.isManagerOrOperator() && (!ctx.auth.authenticated || ctx.auth.agentId !== agentId)) {
      ctx.assertAdminOrOperator();
    }
    const body = await parseBody(req);
    const updated = effectiveRoster.updateAgent(agentId, body);
    sendJson(res, 200, updated);
    return true;
  }
