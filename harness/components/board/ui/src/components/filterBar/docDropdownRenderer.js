...
export function renderDocFilters(filteredTasks) {
  ...
  const openTaskCount = tasks.filter(t => (isStandalone(t) || !isDocClosed(t.design_slug)) && t.status !== 'done').length;
  const standaloneTasks = tasks.filter(isStandalone);
  const openStandaloneCount = standaloneTasks.filter(t => t.status !== 'done').length;
  ...
  let html = item('all', '<span>All open docs</span>', openTaskCount, { lifecycle: 'open', selected: activeDoc === 'all' });
  if (openDocs.length) html += section('Open') + openDocs.map(docItem).join('');
  if (standaloneTasks.length) html += item('standalone', '<span>Standalone tasks</span>', openStandaloneCount, { selected: activeDoc === 'standalone' });
  ...
  let label = `All open docs (${openTaskCount})`;
  if (activeDoc === 'standalone') {
    label = `Standalone (${openStandaloneCount})`;
