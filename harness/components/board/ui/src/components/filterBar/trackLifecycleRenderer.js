...
export function renderTrackFilters() {
  ...
  let scopedTasks = (state.tasks || []).filter(t => (isStandalone(t) || !isDocClosed(t.design_slug)) && t.status !== 'done');
  ...
  const presentTracks = new Set(activeTasks.map(t => t.track).filter(Boolean));
  ...
  if (state.activeTrack !== 'all') {
    ...
    const count = activeTasks.filter(t => t.track === state.activeTrack).length;
  ...
  for (const item of INDUSTRY_TRACKS) {
    ...
    if (presentTracks.has(item.id) || isTopTrack) {
      const count = activeTasks.filter(t => t.track === item.id).length;
  ...
  for (const track of presentTracks) {
    if (!rendered.has(track)) {
      const count = activeTasks.filter(t => t.track === track).length;
